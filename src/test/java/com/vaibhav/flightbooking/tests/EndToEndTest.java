...
public class EndToEndTest extends BaseTest {
	...
	public void verifyUserCanBookFlightSuccessfully() {
		...
		Assert.assertEquals(purchasePage.getSelectedCardType(), "Visa");

		Assert.assertEquals(purchasePage.getCreditCardMonth(), "11");
		purchasePage.clickRememberMe();

		Assert.assertTrue(purchasePage.isRememberMeSelected());
		...
		Assert.assertEquals(confirmationPage.getBookingStatus(), "PendingCapture");
