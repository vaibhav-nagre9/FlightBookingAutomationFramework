...
public class PurchasePageTest extends BaseTest {
	...
	public void verifyFlightIsDisplayed() {
		navigateToPurchasePage();
		PurchasePage purchasePage = new PurchasePage(driver);
		Assert.assertTrue(purchasePage.getFlightNumber().startsWith("Flight Number:"),
				"Flight Number is not displayed correctly");
	...
	public void verifyPrice() {
		navigateToPurchasePage();
		PurchasePage purchasePage = new PurchasePage(driver);
		Assert.assertTrue(purchasePage.getPrice().startsWith("Price:"));
	...
	public void verifyUserCanFillPassengerDetails() {
		...
		purchasePage.fillPassengerDetails("Vaibhav", "India", "Pune", "Maharashtra", "411057");

		Assert.assertEquals(purchasePage.getPassengerName(), "Vaibhav");

		Assert.assertEquals(purchasePage.getPassengerAddress(), "India");

		Assert.assertEquals(purchasePage.getPassengerCity(), "Pune");

		Assert.assertEquals(purchasePage.getPassengerState(), "Maharashtra");

		Assert.assertEquals(purchasePage.getPassengerZipCode(), "411057");
	}
	...
	public void verifyUserCanFillPaymentDetails() {
		...
		purchasePage.fillPaymentDetails("American Express", "1234567891234567", "04", "2030", "Nagre");

		Assert.assertEquals(purchasePage.getSelectedCardType(), "American Express");

		Assert.assertEquals(purchasePage.getCreditCardNumber(), "1234567891234567");

		Assert.assertEquals(purchasePage.getCreditCardMonth(), "04");

		Assert.assertEquals(purchasePage.getCreditCardYear(), "2030");

		Assert.assertEquals(purchasePage.getCardHolderName(), "Nagre");
	}
	...
	public void verifyRememberMeCheckboxCanBeSelected() {
		...
		Assert.assertTrue(purchasePage.isRememberMeSelected(), "Remember Me checkbox is not selected");
	}
